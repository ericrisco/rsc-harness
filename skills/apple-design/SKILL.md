...
**Hand-offs.** Curves, durations and the build itself are `../animate/SKILL.md`'s — this file owns
the **physics and materials**, not the CSS token table. The visual system and brand grounding are
`../design/SKILL.md`'s. Native Swift work is `../swift-ios/SKILL.md`'s (platform) and
`../write-swift/SKILL.md`'s (language); everything here is the *web* translation of Apple's talks.
