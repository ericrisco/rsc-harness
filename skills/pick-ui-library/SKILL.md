...
description: "Pick the frontend library for a task from a curated list: dialogs and popovers, command palette, toasts, one-time-password input, charts, virtualization, drag and drop, state, conditional classNames, variants, dark mode. Use instead of hand-rolling a component or installing an abandoned package."
tags: [frontend, libraries, dependencies, react, ui]
recommends: [react, nextjs, animate]
profiles: [full]
...
**Hand-offs.** Animating what you install is `../animate/SKILL.md`'s. Sonner specifically has its own
guide, `../ask-sonner/SKILL.md`. Framework-level architecture is `../react/SKILL.md`'s and
`../nextjs/SKILL.md`'s — this skill picks the dependency, it does not design the app.
