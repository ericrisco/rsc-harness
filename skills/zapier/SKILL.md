---
name: zapier
description: "Use when operating Zapier from an agent through the official hosted Zapier MCP server — Bearer-key connect, then discover an app's actions, enable one, and invoke a read or write action (or fire a Zap Catch Hook trigger) across 9,000+ apps like Slack, Gmail, Notion, HubSpot by natural language. Triggers: 'connect Zapier MCP to Claude/Cursor', 'run a Zapier action to post to Slack / create a HubSpot contact', 'let my agent search my Gmail through Zapier', 'invoke Zapier actions from my tool', 'how many tasks does a Zapier MCP call cost', 'ejecuta una acción de Zapier desde el agente', 'connecta Zapier MCP al meu agent'. NOT programmatically creating/reading/updating/deleting your own Zaps — Zapier has NO public API for that on a normal account, so say so and route to n8n or make, which have full workflow CRUD APIs. NOT deciding what a flow should be or its error/dedup design (that is automation-flows). NOT a SaaS vendor embedding Zapier via the Partner/Workflow API (rare; see references)."
