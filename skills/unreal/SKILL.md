---
name: unreal
description: "Use when building, scripting, or debugging a game in Unreal Engine 5 (UE5) with Blueprint and/or C++ — gameplay framework (Actor/Pawn/Character/GameMode/PlayerController), Actor lifecycle and tick, UPROPERTY/UFUNCTION/UCLASS reflection and garbage collection, Enhanced Input, dynamic multicast delegates, UMG widgets, Paper2D sprites/flipbooks, and Blueprint↔C++ parity (exposing C++ to Blueprint, calling Blueprint from C++). Triggers: \"UE5\", \"Unreal\", \"Blueprint\", \"make a Character in C++\", \"expose this to Blueprint\", \"BlueprintCallable\", \"why is my UObject garbage collected\", \"Enhanced Input mapping context\", \"UMG widget\", \"Paper2D flipbook\", \".uproject\", \"GameMode vs GameState\". NOT for Godot (→ godot), NOT for Unity/C#/MonoBehaviour (→ unity), NOT for language-only C++ memory/RAII questions with no Unreal types (→ cpp)."
