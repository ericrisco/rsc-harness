...
description: "Reverse glossary: turn a vague description of a motion effect into its exact name — a popover growing from its trigger is origin-aware, iOS overscroll is rubber-banding, items appearing one after another with a delay is a stagger. Use for \"what's it called when…\" or \"what do you call it when…\"."
tags: [motion, animation, glossary, terminology, naming]
recommends: [animate, apple-design]
profiles: [full]
...
_Adapted from the vocabulary page at [animations.dev](https://animations.dev/) (MIT). It is a
snapshot, not a live mirror: this catalog does not control that page, so treat any drift as the
upstream page being newer — and stay inside this glossary rather than inventing a term._
