...
description: "Review, critique or grade existing animation and motion code against a strict craft bar — easing, duration, transform-origin, interruptibility, GPU-only properties, reduced motion, enter/exit asymmetry, stagger. Default is to flag; approval is earned. Use on the transitions in a diff or a component."
tags: [motion, animation, review, craft, ui]
recommends: [animate, improve-animations, code-review]
profiles: [full]
...
**Hand-offs.** Reviewing a whole codebase's motion, and writing plans another agent can execute, is
`../improve-animations/SKILL.md`'s — this skill's object is **one diff or one component**. Finding
motion that is *missing* is `../find-animation-opportunities/SKILL.md`'s. General code review is
`../code-review/SKILL.md`'s; decline non-motion code and point there.

**The countable half of this bar is executable.** `scripts/verify.sh` in this skill carries the tell
registry — `transition: all`, a `scale(0)` entrance, `ease-in` on UI, a centered popover origin, a
layout-property transition, the Motion `x`/`y`/`scale` shorthands, and motion with no reduced-motion
guard anywhere. Run it first (`bash scripts/verify.sh`, add `--strict` to fail on warnings), then
spend your judgement on what it cannot see: frequency tier, purpose, cohesion, and whether the thing
feels right. A finding the script already made is not a finding you discovered.
