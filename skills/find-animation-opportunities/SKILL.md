...
description: "Sweep a UI for what does not animate but should, and reject what should not — each suggestion with an exact curve and duration, plus the rejected candidates. Read-only. Use for 'what could be animated here', 'where would motion help', or an interface that feels static and dead."
tags: [motion, animation, restraint, opportunities, ui]
recommends: [animate, improve-animations]
profiles: [full]
...
**Hand-offs.** Building any suggestion is `../animate/SKILL.md`'s (or `../animate-expo/SKILL.md`'s on
mobile). Judging motion that already exists is `../review-animations/SKILL.md`'s; turning a whole
codebase into plans is `../improve-animations/SKILL.md`'s.
