...
description: "Audit a whole codebase's animation and motion, then write prioritized findings and self-contained plans another agent can execute. Read-only. Use for 'improve the animations', 'make the app feel better', or a roadmap of motion fixes — rather than a verdict on one diff."
tags: [motion, animation, audit, plan, codebase]
recommends: [review-animations, animate, plan]
profiles: [full]
...
**Hand-offs.** Reviewing **one diff** is `../review-animations/SKILL.md`'s — this skill's object is
the **whole codebase**, and the deliverable is plans, not a verdict. Finding motion that is *missing*
is `../find-animation-opportunities/SKILL.md`'s. Building the fix is `../animate/SKILL.md`'s.

Run `../review-animations/scripts/verify.sh` during recon: it gives you the countable findings for
free, so the audit's judgement goes on what a pattern cannot see.
...
The rule catalog with precise values lives in [references/audit.md](references/audit.md). The plan format lives in [references/plan-template.md](references/plan-template.md). Load them when you audit and when you write plans.

## Hard Rules

1. **Never modify source code.** The only files you create or edit are the plans. **Where they go:**
   if the workspace has a harness (`02-DOCS/` exists), write them to `02-DOCS/wiki/sdd/plans/` and
   index them in `02-DOCS/wiki/index.md` — a second place where plans live is a second ledger, and
   the stale one always wins. Otherwise `plans/` at the repo root (or `animation-plans/` if `plans/`
   is already taken). If asked to "just fix it", decline and point to `improve-animations execute <plan>` or to running the plan with any agent.
...
Write for the weakest executor: exact file paths and current-code excerpts, the exact target values (cubic-beziers, durations, spring configs — pulled from `references/audit.md`, never approximated), the repo's own conventions with an exemplar, ordered steps, hard scope boundaries, and a verification section including how to *feel-check* the result (slow motion, frame-by-frame, real device for gestures).
